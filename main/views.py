...
def premium_notes(request):
    try:
        notes = models.BoughtFile.objects.filter(user=request.user).order_by(
            "-bought_at"
        )
...
def notes(request):
    try:
        notes = (
            models.Note.objects.prefetch_related("files").all().order_by("-created_at")
        )
    ...
    context = {"premium_notes": premium_notes, "free_notes": free_notes}
...
def classroom(request, slug):
    ...
    context = {"classroom": classroom, "slug": slug, "posts": posts, "members": members}
...
def home(request):
    if request.user.role == "student":
        ...
        context = {"classrooms": teacher_classrooms}
