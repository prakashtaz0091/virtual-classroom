...
def initiate_payment(request, note_id):
    ...
    try:
        ...
        old_transaction = models.EsewaTransaction.objects.filter(
            notes=note, initiater=request.user
        ).first()
        if old_transaction:
    ...
    payload["signature"] = generate_signature(
        payload, settings.ESEWA_CONFIG["MERCHANT_SECRET"]
    )
    print("our signature", payload["signature"])

    return render(
        request,
        "esewa_payment/payment_form.html",
        {"form_url": settings.ESEWA_CONFIG["FORM_URL"], "payload": payload},
    )
...
def payment_success(request, transaction_uuid):
    ...
    try:
        transaction = models.EsewaTransaction.objects.get(
            transaction_id=payload_transaction_uuid
        )
        if transaction.notes.amount == float(payload_total_amount):
            ...
            print(file_bought)
            BoughtFile.objects.create(user=transaction.initiater, file=file_bought)

            messages.success(
                request, "Payment successful, you can download the file now!"
            )
            ...
            return HttpResponse(
                "Transaction not found, something wrong in payment data", status=404
            )
...
def payment_failure(request, transaction_uuid):
    ...
    try:
        # print(transaction_uuid)
        transaction = models.EsewaTransaction.objects.filter(
...
#     "signature": "qFsY1VDpjRtG8c2Vd/8v4Cwtxtktbb3a1iQMauc2FsI=",
# }
